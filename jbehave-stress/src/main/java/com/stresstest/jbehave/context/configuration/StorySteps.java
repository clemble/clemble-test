 ...
 * </ul>
 * <b>The "has-a" design model, in which the steps instance is passed in, is strongly recommended over the "is-a" model as it does not have tie-ins in the
 * {@link Steps} class implementation</b>.
 * </p>
 * <p>
 * You can define the methods that should be run by annotating them with {@link Given @Given}, {@link When @When} or {@link Then @Then}, and providing as a
 * value for each annotation a pattern matches the textual step. The value is interpreted by the {@link StepPatternParser}, which by default is a
 * {@link RegexPrefixCapturingPatternParser} that interprets the words starting with '$' as parameters.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * The {@link Configuration} can be used to provide customize the {@link StepCandidate}s that are created, e.g. providing a step monitor or creating them in
 * "dry run" mode.
 * </p>
 */
public class StorySteps implements CandidateSteps {
     ...
     */
    public StorySteps(Configuration configuration) {
     ...
     */
    public StorySteps(Configuration configuration, Object instance) {
        this.configuration = configuration;
     ...
     */
    public StorySteps(Configuration configuration, Class<?> type, InjectableStepsFactory stepsFactory) {
        this.configuration = configuration;
        this.type = type;
    ...
    private StepCandidate createCandidate(Method method, StepType stepType, String stepPatternAsString, int priority, Configuration configuration) {
        return new StepCandidate(stepPatternAsString, priority, stepType, method, type, stepsFactory, configuration.keywords(),
                configuration.stepPatternParser(), configuration.parameterConverters(), configuration.parameterControls());
    }
    ...
    private BeforeOrAfterStep createBeforeOrAfterStep(Stage stage, Method method, Outcome outcome) {
        return new BeforeOrAfterStep(stage, method, outcome, new StepCreator(type, stepsFactory, configuration.parameterConverters(),
                configuration.parameterControls(), null, configuration.stepMonitor()));
    }

    private List<Method> allMethods() {
        // Step 1. Preparing responce
        Set<Method> allMethods = new HashSet<Method>();
        // Step 2.0 Going through whole hierarchy of classes and subclasses
        Class<?> target = type;
        while (target != Object.class) {
            allMethods.addAll(asList(target.getMethods()));
            // Step 2.1. Checking all inherited interfaces
            for (Class<?> inheritedInterface : target.getInterfaces())
                allMethods.addAll(asList(inheritedInterface.getMethods()));
            // Step 2.2. Next step check superclass
            target = ((Class<?>) target).getSuperclass();
        }
        // Step 3. Returning accumulated methods
