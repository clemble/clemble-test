...
/**
 * <p>
 * {@code SpringJUnit4ClassRunner} is a custom extension of {@link BlockJUnit4ClassRunner} which provides functionality of the
 * <em>Spring TestContext Framework</em> to standard JUnit 4.5+ tests by means of the {@link TestContextManager} and associated support classes and annotations.
 * </p>
 * <p>
 * The following list constitutes all annotations currently supported directly by {@code SpringJUnit4ClassRunner}.
...
public class SpringJUnit4ClassRunner extends BlockJUnit4ClassRunner {
    ...
    /**
     * Constructs a new {@code SpringJUnit4ClassRunner} and initializes a {@link TestContextManager} to provide Spring testing functionality to standard JUnit
     * tests.
     * 
     * @param clazz
    ...
    /**
     * Creates a new {@link TestContextManager} for the supplied test class and the configured <em>default {@code ContextLoader} class name</em>. Can be
    ...
    /**
     * Get the name of the default {@code ContextLoader} class to use for the supplied test class. The named class will be used if the test class does not
     * explicitly declare a {@code ContextLoader} class via the {@code &#064;ContextConfiguration} annotation.
     * <p>
     * The default implementation returns {@code null}, thus implying use of the <em>standard</em> default {@code ContextLoader} class name. Can be overridden
     * by subclasses.
     * 
     * @param clazz
     *            the test class
    ...
    /**
     * Returns a description suitable for an ignored test class if the test is disabled via {@code &#064;IfProfileValue} at the class-level, and otherwise
    ...
    /**
     * Wraps the {@link Statement} returned by the parent implementation with a {@link RunBeforeTestClassCallbacks} statement, thus preserving the default
    ...
    /**
     * Wraps the {@link Statement} returned by the parent implementation with a {@link RunAfterTestClassCallbacks} statement, thus preserving the default
     * functionality but adding support for the Spring TestContext Framework.
     * 
    ...
    /**
     * {@code springMakeNotifier()} is an exact copy of {@link BlockJUnit4ClassRunner BlockJUnit4ClassRunner's} {@code makeNotifier()} method, but we have
     * decided to prefix it with "spring" and keep it {@code private} in order to avoid the compatibility clashes that were introduced in JUnit between versions
     * 4.5, 4.6, and 4.7.
     */
    ...
    /**
     * Augments the default JUnit behavior {@link #withPotentialRepeat(FrameworkMethod, Object, Statement) with potential repeats} of the entire execution
     * chain.
     * <p>
     ...
     * timeouts will work fine in combination with Spring transactions. Note that JUnit-specific timeouts still differ from Spring-specific timeouts in that the
     * former execute in a separate thread while the latter simply execute in the main thread (like regular tests).
     * 
    ...
    /**
     * Invokes JUnit 4.7's private {@code withRules()} method using reflection. This is necessary for backwards compatibility with the JUnit 4.5 and 4.6
    ...
    private Statement withRulesReflectively(FrameworkMethod frameworkMethod, Object testInstance, Statement statement) {
        Method withRulesMethod = ReflectionUtils.findMethod(getClass(), "withRules", FrameworkMethod.class, Object.class, Statement.class);
        if (withRulesMethod != null) {
            ...
            statement = (Statement) ReflectionUtils.invokeMethod(withRulesMethod, this, frameworkMethod, testInstance, statement);
        }
        return statement;
    }

    /**
     * Returns {@code true} if {@link Ignore &#064;Ignore} is present for the supplied {@link FrameworkMethod test method} or if the test method is disabled via
     * {@code &#064;IfProfileValue}.
     * 
    ...
    /**
     * Get the {@code exception} that the supplied {@link FrameworkMethod test method} is expected to throw.
     * <p>
     * Supports both Spring's {@link ExpectedException @ExpectedException(...)} and JUnit's {@link Test#expected() @Test(expected=...)} annotations, but not
     * both simultaneously.
     * 
    ...
    protected Class<? extends Throwable> getExpectedException(FrameworkMethod frameworkMethod) {
        ...
        Class<? extends Throwable> junitExpectedException = (testAnnotation != null && testAnnotation.expected() != Test.None.class ? testAnnotation.expected()
                : null);
        ...
        if (springExpectedException != null && junitExpectedException != null) {
            String msg = "Test method [" + frameworkMethod.getMethod() + "] has been configured with Spring's @ExpectedException("
                    + springExpectedException.getName() + ".class) and JUnit's @Test(expected=" + junitExpectedException.getName() + ".class) annotations. "
    ...
    /**
     * Supports both Spring's {@link Timed &#064;Timed} and JUnit's {@link Test#timeout() &#064;Test(timeout=...)} annotations, but not both simultaneously.
     * Returns either a {@link SpringFailOnTimeout}, a {@link FailOnTimeout}, or the unmodified, supplied {@link Statement} as appropriate.
     * 
    ...
    protected Statement withPotentialTimeout(FrameworkMethod frameworkMethod, Object testInstance, Statement next) {
        ...
        if (springTimeout > 0 && junitTimeout > 0) {
            String msg = "Test method [" + frameworkMethod.getMethod() + "] has been configured with Spring's @Timed(millis=" + springTimeout
                    + ") and JUnit's @Test(timeout=" + junitTimeout + ") annotations. Only one declaration of a 'timeout' is permitted per test method.";
            logger.error(msg);
            throw new IllegalStateException(msg);
    ...
    /**
     * Wraps the {@link Statement} returned by the parent implementation with a {@link RunBeforeTestMethodCallbacks} statement, thus preserving the default
     * functionality but adding support for the Spring TestContext Framework.
     * 
    ...
    /**
     * Wraps the {@link Statement} returned by the parent implementation with a {@link RunAfterTestMethodCallbacks} statement, thus preserving the default
     * functionality but adding support for the Spring TestContext Framework.
     * 
    ...
    /**
     * Supports Spring's {@link Repeat &#064;Repeat} annotation by returning a {@link SpringRepeat} statement initialized with the configured repeat count or
     * {@code 1} if no repeat count is configured.
     * 
     * @see SpringRepeat
     */
     ...
     * =============================================================================================
     * 
     * Upper is a copy of SpringJUnit4ClassRunner Lower is changes needed to run FrequentRunner
     * 
     * =============================================================================================
     * */
    ...
    /**
     * Check whether the test is enabled in the first place. This prevents classes with a non-matching {@code &#064;IfProfileValue} annotation from running
     * altogether, even skipping the execution of {@code prepareTestInstance()} {@code TestExecutionListener} methods.
     * 
     * @see ProfileValueUtils#isTestEnabledInThisEnvironment(Class)
    ...
    public Object createTest() throws Exception {
        if (createdTestRef.get() == null) {
            synchronized (createdTestRef) {
                if (createdTestRef.get() == null) {
